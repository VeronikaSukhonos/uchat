...
void open_close_menu(GtkWidget *menu_button, gpointer data) {
  ...
  gtk_stack_set_visible_child_name(GTK_STACK((*main_page).menu_stack), (*main_page).menu_opened == 1 ? "chats_list" : "menu");
  (*main_page).menu_opened *= -1;
}

void show_new_chat(GtkWidget *new_chat_button, gpointer data) {
  t_main_page_data *main_page = (t_main_page_data *)data;
  gtk_stack_set_visible_child_name(GTK_STACK((*main_page).central_area_stack), "create_chat");
}

void show_new_group(GtkWidget *new_group_button, gpointer data) {
  t_main_page_data *main_page = (t_main_page_data *)data;
  gtk_stack_set_visible_child_name(GTK_STACK((*main_page).central_area_stack), "create_group");
}
...
void chat_creation(GtkWidget *create_chat_button, gpointer data) {
  t_main_page_data *main_page = (t_main_page_data *)data;
  char *username = (char *)gtk_entry_get_text(GTK_ENTRY((*main_page).create_chat_data.username));

  if (check_form_data(username, NULL, (*main_page).create_chat_data.message) == 1) {
    if (strcmp(username, "no") == 0) // check is there such user in database
      ...
      (*main_page).menu_opened = -1;
      gtk_stack_set_visible_child_name(GTK_STACK((*main_page).central_area_stack), "chat");
    }
  }
}
...
void adding_user(GtkWidget *add_user_button, gpointer data) {
  t_main_page_data *main_page = (t_main_page_data *)data;
  char *username = (char *)gtk_entry_get_text(GTK_ENTRY((*main_page).create_group_data.username));

  if (check_form_data(username, NULL, (*main_page).create_group_data.message) == 1) {
    if (strcmp(username, "no") == 0) // check is there such user in database
      gtk_label_set_text(GTK_LABEL((*main_page).create_chat_data.message),
                         "User does not found");
    else {
...
void group_creation(GtkWidget *create_group_button, gpointer data) {
  t_main_page_data *main_page = (t_main_page_data *)data;
  char *name = (char *)gtk_entry_get_text(GTK_ENTRY((*main_page).create_group_data.name));
...
void create_chats_page(GtkWidget *pages, GtkWidget *chats, t_main_page_data *main_page) {
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(menu_button), "dots");
  g_signal_connect(menu_button, "clicked", G_CALLBACK(open_close_menu), main_page);

  GtkWidget *menu_label = gtk_label_new("gChats");
  gtk_grid_attach(GTK_GRID(sidebar), menu_label, 1, 0, 1, 1);
  gtk_style_context_add_class(gtk_widget_get_style_context(menu_label), "gchats");
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(new_chat_button), "menu-button");
  g_signal_connect(new_chat_button, "clicked", G_CALLBACK(show_new_chat), main_page);
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(settings_button), "menu-button");
  ...
  g_signal_connect(log_out_button, "clicked", G_CALLBACK(log_out), NULL); // instead of NULL it must be pages

  chats_list = gtk_label_new("chats_list");
  gtk_stack_add_named(GTK_STACK((*main_page).menu_stack), chats_list, "chats_list");
  ...
  gtk_stack_add_named(GTK_STACK((*main_page).central_area_stack), create_chat, "create_chat");

  (*main_page).create_chat_data.form = gtk_grid_new();
  gtk_box_pack_start(GTK_BOX(create_chat), (*main_page).create_chat_data.form, FALSE, FALSE, 50);
  gtk_style_context_add_class(gtk_widget_get_style_context((*main_page).create_chat_data.form), "newchats");
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(create_chat_username_label), "form-label");
  gtk_grid_attach(GTK_GRID((*main_page).create_chat_data.form), create_chat_username_label, 0, 0, 2, 1);
  gtk_widget_set_halign(create_chat_username_label, GTK_ALIGN_START);

  (*main_page).create_chat_data.username = gtk_entry_new();
  gtk_style_context_add_class(gtk_widget_get_style_context((*main_page).create_chat_data.username), "form-entry");
  ...
  gtk_grid_attach(GTK_GRID((*main_page).create_chat_data.form), create_chat_button, 1, 1, 1, 1);
  gtk_style_context_add_class(gtk_widget_get_style_context(create_chat_button), "newchats-button");
  g_signal_connect(create_chat_button, "clicked", G_CALLBACK(chat_creation), main_page);

  (*main_page).create_chat_data.message = gtk_label_new("");
  gtk_grid_attach(GTK_GRID((*main_page).create_chat_data.form),
                  (*main_page).create_chat_data.message, 0, 2, 2, 1);
  gtk_style_context_add_class(gtk_widget_get_style_context((*main_page).create_chat_data.message), "form-message");

  // create group
  create_group = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0);
  gtk_stack_add_named(GTK_STACK((*main_page).central_area_stack), create_group, "create_group");

  (*main_page).create_group_data.form = gtk_grid_new();
  gtk_box_pack_start(GTK_BOX(create_group), (*main_page).create_group_data.form, FALSE, FALSE, 50);
  gtk_style_context_add_class(gtk_widget_get_style_context((*main_page).create_group_data.form), "newchats");
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(create_group_label), "newgroup-label");
  gtk_grid_attach(GTK_GRID((*main_page).create_group_data.form), create_group_label, 0, 0, 2, 1);
  gtk_widget_set_halign(create_group_label, GTK_ALIGN_START);

  GtkWidget *group_name_label = gtk_label_new("Group name");
  gtk_style_context_add_class(gtk_widget_get_style_context(group_name_label), "form-label");
  gtk_grid_attach(GTK_GRID((*main_page).create_group_data.form), group_name_label, 0, 1, 2, 1);
  gtk_widget_set_halign(group_name_label, GTK_ALIGN_START);

  (*main_page).create_group_data.name = gtk_entry_new();
  gtk_style_context_add_class(gtk_widget_get_style_context((*main_page).create_group_data.name), "form-entry");
  ...
  gtk_style_context_add_class(gtk_widget_get_style_context(add_user_label), "form-label");
  gtk_grid_attach(GTK_GRID((*main_page).create_group_data.form), add_user_label, 0, 3, 2, 1);
  gtk_widget_set_halign(add_user_label, GTK_ALIGN_START);

  (*main_page).create_group_data.username = gtk_entry_new();
  gtk_style_context_add_class(gtk_widget_get_style_context((*main_page).create_group_data.username), "form-entry");
  gtk_grid_attach(GTK_GRID((*main_page).create_group_data.form),
                  (*main_page).create_group_data.username, 0, 4, 1, 1);

  GtkWidget *add_user_button = gtk_button_new_with_label("Add");
  gtk_grid_attach(GTK_GRID((*main_page).create_group_data.form), add_user_button, 1, 4, 1, 1);
  ...
  gtk_grid_attach(GTK_GRID((*main_page).create_group_data.form), create_group_button, 0, 6, 2, 1);
