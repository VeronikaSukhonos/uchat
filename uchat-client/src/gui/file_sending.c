...
void save_file(GtkWidget *save_file_button, MessageNode *temp_node) {
	GtkWidget *choose_dst_dialog = gtk_file_chooser_dialog_new(
		"Save a File", GTK_WINDOW(temp_node->message->main_window), GTK_FILE_CHOOSER_ACTION_SAVE,
	...
	while (res != GTK_RESPONSE_CANCEL) {
		...
		if (res == GTK_RESPONSE_ACCEPT) {
			...
			GError *error = NULL;
			gboolean copy_res = g_file_copy(src, dst, G_FILE_COPY_BACKUP,
			...
			if (copy_res == 0 && error != NULL && error->code == G_IO_ERROR_EXISTS) {
				...
				GtkWidget *replace_dialog
					= gtk_message_dialog_new(GTK_WINDOW(choose_dst_dialog),
				...
				if (replace_res == GTK_RESPONSE_YES) {
					copy_res = g_file_copy(src, dst, G_FILE_COPY_OVERWRITE,
										   NULL, NULL, NULL, NULL);
					if (copy_res)
						g_print("Saved file %s to %s\n", temp_node->message->voice_path, filepath);
					else
