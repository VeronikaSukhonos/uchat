...
void change_password(GtkWidget *change_button, gpointer data) {
    ...
    char *old_pw = (char *)gtk_entry_get_text(GTK_ENTRY((*main_page).change_pw.old_pw));
    char *new_pw = (char *)gtk_entry_get_text(GTK_ENTRY((*main_page).change_pw.new_pw));
    char *new_pw_again = (char *)gtk_entry_get_text(GTK_ENTRY((*main_page).change_pw.new_pw_again));
    if (strcmp(new_pw, new_pw_again) != 0) {
        gtk_label_set_text(GTK_LABEL((*main_page).change_pw.message), "Passwords do not match.");
        return;
    }
    ...
    if (check_password(new_pw, (*main_page).change_pw.message)) {
        ...
        if (send(main_page->sock, json_str, strlen(json_str), 0) == -1) {
            success_or_error_msg(main_page->change_pw.message, "Failed to change password. Please try again.", FALSE);
            g_free(json_str);
            return;
        }
        success_or_error_msg(main_page->change_pw.message, "Password changed successfully!", TRUE);
...
void change_email(GtkWidget *change_button, gpointer data) {
    ...
    if (check_email(email, (*main_page).email_change.message)) {
        ...
        free(json_str);
        if (send(main_page->sock, json_str, strlen(json_str), 0) == -1) {
            success_or_error_msg(main_page->email_change.message, "Failed to change email. Please try again.", FALSE);
            g_free(json_str);
            return;
        }
        main_page->email_change.email = gtk_entry_new();
        gtk_entry_set_text(GTK_ENTRY(main_page->email_change.email), email);
        success_or_error_msg(main_page->email_change.message, "Email changed successfully!", TRUE);
...
void show_email(GtkWidget *support_button, gpointer data) {
    ...
    gtk_stack_set_visible_child_name(GTK_STACK((*main_page).central_area_stack), "email");
}
void show_support(GtkWidget *support_button, gpointer data) {
    ...
    gtk_stack_set_visible_child_name(GTK_STACK((*main_page).central_area_stack), "support");
}
...
gboolean delayed_page_switch(gpointer data) {
    ...
    gtk_stack_set_visible_child_name(GTK_STACK(main_page->central_area_stack), main_page->current_page);
...
void send_support_request(GtkWidget *support_button, gpointer data) {
    t_main_page_data *main_page = (t_main_page_data *)data;
    const gchar *support_title = gtk_combo_box_text_get_active_text(GTK_COMBO_BOX_TEXT(main_page->support.subject_combo));
    const gchar *support_message = gtk_entry_get_text(GTK_ENTRY(main_page->support.support_request));
    const gchar *email = gtk_entry_get_text(GTK_ENTRY(main_page->email_change.email));
    if (g_strcmp0(support_title, "") == 0 || g_strcmp0(support_message, "") == 0) {
        success_or_error_msg(main_page->support.message, "Please fill in all fields before submitting.", FALSE);
        return;
    }
    if (email == NULL || strcmp(email, "") == 0) {
        success_or_error_msg(main_page->support.message, "Please submit your email first.", FALSE);
    ...
    cJSON_AddStringToObject(support_data, "title", support_title ? support_title : "");
    cJSON_AddStringToObject(support_data, "message", support_message ? support_message : "");
    ...
    if (send(main_page->sock, json_str, strlen(json_str), 0) == -1) {
        success_or_error_msg(main_page->support.message, "Failed to submit the support request. Please try again.", FALSE);
        g_free(json_str);
        return;
    }
    ...
    gtk_combo_box_set_active(GTK_COMBO_BOX(main_page->support.subject_combo), -1); // Reset combo box
    gtk_entry_set_text(GTK_ENTRY(main_page->support.support_request), "");

    success_or_error_msg(main_page->support.message, "Support request submitted successfully!", TRUE);
}
...
gboolean check_email(const char *email, GtkWidget *message_label) {
    ...
    if (!at_sign) {
        success_or_error_msg(message_label, "Email must contain an '@' symbol.", FALSE);
        return FALSE;
    }
    ...
    if (strlen(domain) == 0) {
        success_or_error_msg(message_label, "Email must have a domain after '@'.", FALSE);
        return FALSE;
    }

    const char *dot = strchr(domain, '.');
    if (!dot) {
        success_or_error_msg(message_label, "Email domain must contain a '.' symbol.", FALSE);
        return FALSE;
    }

    const char *last_dot = strrchr(domain, '.');
    if (strlen(last_dot + 1) < 2) {
        success_or_error_msg(message_label, "Domain extension must have at least 2 characters.", FALSE);
        return FALSE;
    }

    for (const char *c = email; *c; ++c) {
        if (!isalnum(*c) && *c != '@' && *c != '.' && *c != '-' && *c != '_') {
            success_or_error_msg(message_label, "Email contains invalid characters.", FALSE);
            return FALSE;
        }
    }
