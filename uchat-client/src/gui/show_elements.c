        ...
        if (json) {
          ...
          if (cJSON_IsArray(members)) {
            ...
            cJSON_ArrayForEach(member, members) {
              ...
              if (username_json && strcmp(username_json->valuestring, username) != 0) {
...
void show_participant_profile(GtkWidget *profile_icon, gpointer data) {
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(content_box), "profile-content");

    // Profile picture
    GtkWidget *avatar = gtk_image_new_from_file("uchat-client/src/gui/resources/rabbit_profile.png");
    ...
    gtk_style_context_add_class(gtk_widget_get_style_context(username_label), "profile-username");

    // Status
    GtkWidget *status_label = gtk_label_new("online");
    gtk_box_pack_start(GTK_BOX(content_box), status_label, FALSE, FALSE, 0);
    gtk_style_context_add_class(gtk_widget_get_style_context(status_label), "profile-status");
    ...
    if (main_page->chat_nickname) {
        cJSON_AddStringToObject(json, "username", 
                               gtk_label_get_text(GTK_LABEL(main_page->chat_nickname)));
    }
    char *json_str = cJSON_Print(json);
    cJSON_Delete(json);
