...
int handle_update_message(sqlite3 *db, Client *client, cJSON *json,
  ...
  if (!cJSON_IsNumber(message_id_json) || !cJSON_IsString(content_json)) {
  ...
  update_message(db, message_id, content);
  ...
  cJSON_AddStringToObject(response, "action", "UPDATE_MESSAGE_FROM_CHAT");
  ...
  cJSON_AddStringToObject(sender_response, "action", "UPDATE_MESSAGE_STATUS");
