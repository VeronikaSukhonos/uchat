...
void handle_call_forward(Client clients[], Client *client, cJSON *json,
  ...
  // Update in_call_with for the caller and callee
  strcpy(client->in_call_with, callee_name->valuestring);
  if (callee_index != -1) {
    strcpy(clients[callee_index].in_call_with, client->username);
  }
  ...
  if (send(target_sock, response_str, strlen(response_str), 0) == -1) {
    perror("Failed to send call request to callee");
  }
